...
public class LibraryService {
    ...
    public void cadastrarLivro(Livro livro, Long usuarioId) {
        ...
        if (usuarioEmprestimoNaoEntregues(usuario)) {
    ...
    private boolean usuarioEmprestimoNaoEntregues(Usuario usuario) {
        return usuario.getEmprestimos().stream().anyMatch(e ->!e.isEntregaRealizada());
    }
    ...
    public String realizarEmprestimo(Long usuarioId, Long livroId) {
        Usuario usuario = userRepository.findById(usuarioId).orElseThrow(() -> new IllegalArgumentException("Usuário não encontrado"));
    ...
    public String realizarDevolucao(Long usuarioId, Long livroId) {
        ...
        return "Atenção: Você não possui esse livro emprestado